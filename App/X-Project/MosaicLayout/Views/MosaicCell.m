...
//        [operation start];
        ...
        UIImage *cachedImage = [[SDImageCache sharedImageCache] imageFromDiskCacheForKey:_mosaicData.imageFilename];
        if ( cachedImage ) {
            self.image = image;
        }
        else {
            [_imageView sd_setImageWithURL:anURL completed:^(UIImage *image1, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                if ( image1 ) {
                    [[SDImageCache sharedImageCache] storeImage:image1 forKey:_mosaicData.imageFilename toDisk:YES];
                    self.image = image1;
